...
- (id)passUrl:(NSString *)urlstr tiemStamp:(NSNumber *)timestampvalue andParaDic:(NSDictionary *)paraDicOrigin withMethod:(AccessMethod)accessMehod andRequestId:(NSNumber *)requestId thenFilterKey:(NSString *)filterKey useSyn:(BOOL)isSyn dataForm:(NSUInteger)dataForm
{
    ...
    else
    {
        ...
        // 设置body
        [self setPostBody:paraDic giv:giv aeskey:aeskey request:request];
        // 设置method
...
//                        [[Coordinator temptUserProfile]setAccessToken:X_TK];
                ...
                HSLog(@"post string:%@", responseString);
            }
            else
                responseString = nil;
 ...
 *	@param 	request 	<#request description#>
 */
- (void)setPostBody:(NSMutableDictionary *)paraDic giv:(NSString *)giv aeskey:(NSString *)aeskey request:(__weak ASIHTTPRequest *)request

{
    ...
    request.postBody = [NSMutableData dataWithData:compressedPostBody];
